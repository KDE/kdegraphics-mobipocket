...
void DecompressorTest::testNoop()
{
    ...
    // The PDB file requires a QIODevice, but arbitrary data is sufficient
    QBuffer dummyDevice;
    dummyDevice.setData(QByteArray(0x4e, '\0'));
    dummyDevice.open(QBuffer::ReadOnly);

    PDB dummyFile(&dummyDevice);

    auto decompressor = Decompressor::create(1, dummyFile);
...
void DecompressorTest::testRLE()
{
    ...
    auto decompressor = Decompressor::create(2, dummyFile);
